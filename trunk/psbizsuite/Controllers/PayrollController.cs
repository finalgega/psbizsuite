...
namespace psbizsuite.Controllers
{
    public class PayrollController : Controller
    {
        ...
        public ActionResult Calculate()
        {
            ...
            var model = db.Database.SqlQuery<CalculatePayrollViewModel>("SELECT Employee_UserAccount_Username as name, SEC_TO_TIME(Sum(TIME_TO_SEC(TIMEDIFF(ClockOutTime,ClockInTime)))) as totalHours, SEC_TO_TIME(Sum(TIME_TO_SEC(TIMEDIFF(EndShirtHour,StartShiftHour)))) as normalHours, SalaryPerHour as salaryPerHour FROM Attendance a INNER JOIN employee e ON a.Employee_UserAccount_Username = e.UserAccount_Username GROUP BY Employee_UserAccount_Username").ToList();
